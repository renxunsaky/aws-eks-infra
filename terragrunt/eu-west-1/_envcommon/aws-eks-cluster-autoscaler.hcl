terraform {
  source = "${local.base_source_url}//?ref=v2.0.0"
}
...
locals {
  base_source_url = "git::https://github.com/lablabs/terraform-aws-eks-cluster-autoscaler.git"
...
inputs = {
  ...
  helm_chart_version               = "9.19.2"
  helm_description                 = "Terraform deployed helm release for AWS EKS cluster auto scaler"
  irsa_role_name_prefix            = "${local.env.project}-${local.env.env}-eks"
